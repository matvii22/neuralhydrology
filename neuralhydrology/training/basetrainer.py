    ...
    def initialize_training(self):
        ...
        if self.cfg.target_noise_std is not None:
            ...
            self._target_mean = torch.from_numpy(
                ds.scaler["xarray_feature_center"][self.cfg.target_variables].to_array().values).to(dtype=torch.float32, device=self.device)
            self._target_std = torch.from_numpy(
                ds.scaler["xarray_feature_scale"][self.cfg.target_variables].to_array().values).to(dtype=torch.float32, device=self.device)
